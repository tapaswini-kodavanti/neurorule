...
from leaf_common.session.population_session_factory import PopulationSessionFactory

from esp_sdk.session.esp_service_population_session import EspServicePopulationSession


# pylint: disable=too-few-public-methods
class EspPopulationSessionFactory(PopulationSessionFactory):
    """
    An implementation PopulationSessionFactory for creating PopulationSession
    objects used to communicate with a Population Service and/or underlying algorithm.
    """
    ...
    def create_session(self, population_service_host: str,
                       ...
                       connection_options: dict = None) -> Optional[PopulationSession]:
        """
        ...
        # Create the session using the esp service, if defined
        if population_service_host:

            session = EspServicePopulationSession(population_service_host, population_service_port,
                                                  timeout_in_seconds, service, service_routing,
                                                  metadata, security_cfg, connection_options)
            if not session.is_valid:
                # Creation of the session failed, e.g. because of invalid Auth0 credentials.
                # We do not have a session. Return None.
                session = None
        else:
            # Import the EspDirectPopulationSession inline so people without
            # access to it can still use the PopulationSessionFactory class.
            from esp_service.session.direct_esp_population_session \
                import DirectEspPopulationSession   # pylint: disable=import-outside-toplevel,import-error

            # DEF: Need some real values for other args, or have defaults in the implementation
            session = DirectEspPopulationSession(service_start_time=self.SERVICE_START_TIME,
                                                 scm_version="esp_sdk bundle",
