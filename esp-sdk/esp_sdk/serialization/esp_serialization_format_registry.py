...
from esp_sdk.serialization.keras_nn_serialization_format import KerasNNSerializationFormat
from esp_sdk.serialization.nn_weights_serialization_format import NNWeightsSerializationFormat


class EspSerializationFormatRegistry(RepresentationSerializationFormatRegistry):
    """
    Registry class for SerializationFormats for ESP registration types
    """
    ...
    def __init__(self):
        ...
        if self.__class__.registry is None:
            ...
            self.__class__.registry = RepresentationSerializationFormatRegistry()

            # Register some esp-specifics to the singleton
            self.register(RepresentationType.KerasNN, KerasNNSerializationFormat())
            self.register(RepresentationType.NNWeights, NNWeightsSerializationFormat())

    def get_for_representation_type(self, rep_type: RepresentationType) -> SerializationFormat:
        """
    ...
    def reset(self):
        """
        Resets the singleton, which also clears any caches.
        """
